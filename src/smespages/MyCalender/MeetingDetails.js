...
const handleAccept = async () => {
  ...
  try {
    ...
    if (!meeting?.date) {
      throw new Error("Meeting date is missing");
    }

    if (!meeting?.timeSlots?.length) {
      throw new Error("No time slots available");
    }

    if (!meeting.smeAppId) {
      throw new Error("SME application ID missing");
    }

    // 2. Get the application documents
    const smeAppRef = doc(db, "smeApplications", meeting.smeAppId);
    const smeAppSnap = await getDoc(smeAppRef);

    if (!smeAppSnap.exists()) {
      throw new Error("SME application not found");
    }

    const appData = smeAppSnap.data();
    const acceptedDate = meeting.date.toISOString();

    // 3. Validate availableDates exists and is an array
    if (!Array.isArray(appData.availableDates)) {
      throw new Error("Available dates not properly formatted");
    }

    // 4. Find the accepted availability with proper null checks
    const acceptedAvailability = appData.availableDates.find(avail => {
      return avail?.date && avail.date === acceptedDate;
    });

    if (!acceptedAvailability) {
      throw new Error("Selected date not found in available dates");
    }

    // 5. Validate timeSlots exists
    if (!Array.isArray(acceptedAvailability.timeSlots)) {
      throw new Error("Time slots not properly formatted");
    }

    // 6. Get investor application reference
    let investorAppRef;
    if (meeting.investorAppId) {
      investorAppRef = doc(db, "investorApplications", meeting.investorAppId);
    } else if (appData.funderId) {
      // Fallback: Find investor application by querying
      const investorAppsQuery = query(
        collection(db, "investorApplications"),
        where("funderId", "==", appData.funderId),
        where("smeId", "==", appData.smeId)
      );
      const investorAppsSnapshot = await getDocs(investorAppsQuery);
      if (!investorAppsSnapshot.empty) {
        investorAppRef = doc(db, "investorApplications", investorAppsSnapshot.docs[0].id);
      }
    }

    if (!investorAppRef) {
      throw new Error("Could not find investor application");
    }

    // 7. Prepare updates
    const updates = {
      availableDates: [acceptedAvailability],
      meetingStatus: 'scheduled',
      scheduledMeeting: {
        date: acceptedDate,
        timeSlot: acceptedAvailability.timeSlots[0],
        location: appData.meetingLocation || meeting.location,
        purpose: appData.meetingPurpose || meeting.title,
        timeZone: acceptedAvailability.timeZone || meeting.timeZone
      },
    ...
    // 8. Execute updates
    await Promise.all([
      updateDoc(smeAppRef, updates),
      updateDoc(investorAppRef, updates)
    ]);

    // 9. Create calendar event
    await addDoc(collection(db, "smeCalendarEvents"), {
      smeId: meeting.smeId,
      funderId: appData.funderId,
      title: appData.meetingPurpose || meeting.title,
      date: acceptedDate,
      timeSlot: acceptedAvailability.timeSlots[0],
      location: appData.meetingLocation || meeting.location,
      type: "meeting",
      ...
      applicationId: meeting.smeAppId,
      timeZone: acceptedAvailability.timeZone || meeting.timeZone
    });

    // 10. Success
    showResponseMessage('Meeting confirmed successfully!');
    if (onAction) {
      onAction(meeting.id, 'scheduled');
    }
  } catch (error) {
         ...
         <div className="meeting-info">
          ...
          {showResponse ? (
            ...
            <div className="meeting-actions">
              ...
              >
                {isProcessing ? 'Processing...' : 'Confirm Meeting'}
              </button>
              <button 
