    ...
    const unsubscribe = onSnapshot(q, async (querySnapshot) => {
      ...
      for (const doc of querySnapshot.docs) {
        ...
        if (data.availableDates && data.availableDates.length > 0) {
          // Get the corresponding investor application
          ...
          const investorAppId = investorAppsSnapshot.empty ? null : investorAppsSnapshot.docs[0].id;

          data.availableDates.forEach((availability) => {
            meetingsData.push({
              id: `${doc.id}-${availability.date}`,
              smeAppId: doc.id,  // ID from smeApplications
              investorAppId: investorAppId,  // ID from investorApplications
  ...
  try {
    if (action === 'scheduled') {
      ...
      const updatedMeetings = meetings.map(meeting => {
        ...
        if (meeting.applicationId === scheduledMeeting.applicationId && meeting.id !== id) {
          return null;
        }
        return meeting;
      }).filter(Boolean);
