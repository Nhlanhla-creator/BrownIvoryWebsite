...
import BillingInfoInvestors from "./billing-info-investors"
import "./investors-subscriptions.css"

const InvestorsSubscriptions = () => {
  const publicKey = "pk_test_e99e9d341c8fa3182737cd26c5838dece90e3ed9"

  // Updated plan configuration based on new table
  const plans = {
    basic: {
      name: "Basic",
      price: 0,
      ...
      features: {
        ...
        "Success Fee (% of transaction value)": "2%",
        "Access to Passive Matches (Browse SMEs)": false,
        "Advanced Filters (e.g. stage, sector)": false,
        "BIG Score Access (per SME)": false,
        "BIG Score Comparison & Benchmarking": false,
        "Priority Dashboard Analytics & Reports": false,
        "Featured DealFlow Placement": false,
        "Custom Alerts & Early Access to Matches": false,
      },
    },
    standard: {
      name: "Standard",
      ...
      features: {
        ...
        "Success Fee (% of transaction value)": "1.5%",
        "Access to Passive Matches (Browse SMEs)": true,
        "Advanced Filters (e.g. stage, sector)": true,
        "BIG Score Access (per SME)": true,
        "BIG Score Comparison & Benchmarking": false,
        "Priority Dashboard Analytics & Reports": true,
        "Featured DealFlow Placement": false,
        "Custom Alerts & Early Access to Matches": false,
      },
    },
    premium: {
      ...
      currency: "ZAR",
      features: {
        "Subscription Fee": "ZAR 500",
        "Success Fee (% of transaction value)": "1%",
        "Access to Passive Matches (Browse SMEs)": true,
        "Advanced Filters (e.g. stage, sector)": true,
        "BIG Score Access (per SME)": true,
        "BIG Score Comparison & Benchmarking": true,
        "Priority Dashboard Analytics & Reports": true,
        "Featured DealFlow Placement": true,
        "Custom Alerts & Early Access to Matches": true,
      },
    },
  }
  ...
  const featureOrder = [
    ...
    "Access to Passive Matches (Browse SMEs)",
    "Advanced Filters (e.g. stage, sector)",
    "BIG Score Access (per SME)",
    "BIG Score Comparison & Benchmarking",
    "Priority Dashboard Analytics & Reports",
    "Featured DealFlow Placement",
    "Custom Alerts & Early Access to Matches",
  ]
  ...
  const validate = () => {
    ...
    // Only require billing info for paid plans
  ...
  const componentProps = {
    ...
    publicKey,
    text: `Confirm & Pay ZAR ${plans[selectedPlan].price}.00`,
    ...
    <div className="container">
      ...
      <div className="plan-grid">
        {Object.entries(plans).map(([planKey, plan]) => (
          ...
          >
            ...
            {/* Show success fee prominently for each plan */}
            <div className="success-fee-text">
              + {plan.features["Success Fee (% of transaction value)"]} success fee
            </div>
      ...
      {!showBillingInfo && (
        <div className="form-container">
          <h2 className="form-title">{selectedPlan === "basic" ? "Account Information" : "Billing Information"}</h2>
          ...
          <div className="summary-box">
            <div className="summary-content">
              <div className="plan-summary">
                ...
                <p>
                  {selectedPlan === "basic"
                    ? "Free forever + 2% success fee"
                    : `${plans[selectedPlan].currency} ${plans[selectedPlan].price}.00/month + ${plans[selectedPlan].features["Success Fee (% of transaction value)"]} success fee`}
                </p>
              </div>
              <div className="price-summary">
                ...
                <p className="total-price">
                  {selectedPlan === "basic"
                    ? "Free"
                    : `${plans[selectedPlan].currency} ${plans[selectedPlan].price}.00`}
                </p>
              </div>
            </div>
          </div>
      ...
      {/* Transaction History */}
      {history.length > 0 && !showBillingInfo && (
              ...
              <tbody>
                {history.map((entry) => (
                  <tr key={entry.id}>
                    ...
                    <td>{entry.email}</td>
                    <td className="transaction-plan">{entry.plan}</td>
                    <td className="transaction-amount">{entry.amount === 0 ? "Free" : `ZAR ${entry.amount}.00`}</td>
                    ...
                    <td className="transaction-details">
                      {entry.reason ? (
                        <span className="transaction-reason">{entry.reason}</span>
                      ) : entry.status === "Success" || entry.status === "Active" ? (
                        <span className="transaction-success-text">✓ Completed successfully</span>
                      ) : (
                        <span>-</span>
                      )}
                    </td>
